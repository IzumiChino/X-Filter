...
| 5 | **ML 通道** | 31 维基础特征 + 256 维 n-gram hashing 的在线逻辑回归（sigmoid + L2 正则 + SGD），训练样本不足或正/负样本不平衡时自动回退 |
| 6 | **Jaccard 回退** | n-gram + Jaccard 相似度对比黑/白样本库 |

### ML 特征（31 维 + hashing）

包含文本长度、emoji 比、数字比、内容风险分数、URL、字符多样性、重复度、handle 数字/熵/尾随数字、名字 emoji 比、是否回复、是否含媒体、与黑/白样本的 Jaccard、handle 历史声誉、粉丝数、关注数、粉关比、有无 bio、是否认证、联系方式信号、约炮/色情服务锚点、诈骗/返利/博彩/贷款锚点、中英混排比例、强风险锚点、软诱导弱信号、账号可疑度、软诱导组合信号等。

v5.1 去掉了“中文本身就是特征”的设计，改成只把中文内容里的明确风险锚点作为正向信号；普通中文文本会提高 ML / Jaccard 判定阈值，降低一标垃圾后“中文全变垃圾”的误伤。

v5.2 增加软诱导组合判定，用来处理“刚分手 / 想被爱 / 陪陪我”等更像真人发言的垃圾回复。相关词只作为弱信号，不会单独触发屏蔽，必须同时叠加账号可疑度、回复区/上下文相似、短文本/emoji 等条件。
...
- 模式开关：标记模式 / ML / 启发式 / 软诱导组合 / 关注免屏蔽 / 结构通道 / 调试日志
- ML 参数：回复区阈值、时间线阈值、最小训练样本数
- 启发式参数：回复区/时间线阈值、软诱导组合阈值
